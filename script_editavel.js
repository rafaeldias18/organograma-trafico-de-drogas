...
function renderizarOrganograma() {
    ...
    const statusMap = {
        ...
        'temporaria': 'Temporária' // Alterado para "Temporária"
    };
    ...
    container.innerHTML = individuosFiltrados.map(individuo => {
        ...
        const statusClass = individuo.status.replace(/\s+/g, '-').toLowerCase(); // Garante que "temporaria" vira "temporaria"
        const statusExibicao = statusMap[individuo.status] || individuo.status; 
        ...
        return `
            <div class="individual-card status-${statusClass}" onclick="abrirModalDetalhes(${individuo.id})">
                ...
                <div class="card-info">
                    ...
                    <span class="status-badge ${statusClass}">${statusExibicao}</span>
...
function abrirModalDetalhes(id) {
    ...
    // Mapeamento de status para exibição no modal
    const statusMap = {
        'ativo': 'Ativo',
        'preso': 'Preso',
        'tornozeleira': 'Tornozeleira',
    ...
    statusBadge.textContent = statusExibicao; // Exibe o nome formatado
    ...
    if (individuo.ligacoes && individuo.ligacoes.length > 0) {
        ...
        ligacoesContainer.innerHTML = '<div class="ligacao-item">Nenhum vínculo registrado</div>';
    }
...
function abrirModalEdicao(id = null) {
    ...
    // Definir as opções de status no select do modal de edição
    ...
    editStatusSelect.value = individuo.status || 'ativo'; // Seleciona o status correto ou 'ativo' por padrão
